...
class AccountMainDao(BaseDao):
    ...
    def add(self, account_main: AccountMain):
        ...
        with self.session() as sess:
            try:
                ...
                if str(exception.orig)[48:48 + len("unique_account_email")] == 'unique_account_email':
                    return None, ErrorEnum.account_already_exists
                else:
                    raise TypeError
    ...
    def get_by_email_and_hash_password(self, account_main: AccountMain):
        with self.session() as sess:
            row = sess.query(
                ...
                and_(
                    AccountMain._hash_password == account_main.hash_password,
                    AccountMain._email == account_main.email
                )
            ).first()
        if not row:
            return None, ErrorEnum.account_not_found
    ...
    def update_password(self, account_main_id: int, account_main: AccountMain):
        with self.session() as sess:
            account_main_db = sess.query(AccountMain).where(
                AccountMain._id == account_main_id
            ).first()
