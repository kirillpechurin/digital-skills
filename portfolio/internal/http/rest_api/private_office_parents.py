...
from flask import Blueprint, request, json, make_response, render_template, url_for, flash, jsonify
...
from portfolio.internal.http.rest_api.answers.children import get_response_list_children_and_events, \
    get_response_list_children, get_response_detail_children
...
def index(auth_account_main_id: int, parent_id: int):
    if request.method == 'GET':
        ...
        return get_response_list_children_and_events(context)
...
def progress(children_id: int, auth_account_main_id: int, parent_id: int):
    if request.method == 'GET':
        if children_id == 'A':
            ...
            return get_response_list_children(list_children)
        else:
            children = Children(
                id=children_id
            )
            ...
            dict_for_ser = {
                ...
                "complete_events": list_completed_events,
                "list_organisation_for_child": list_organisation_for_child,
            }
            return get_response_detail_children(dict_for_ser)
